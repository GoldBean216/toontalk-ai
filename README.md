...
- 已获取 [Google AI Studio Gemini API Key](https://aistudio.google.com/)
...
3. **配置环境变量**
   复制项目根目录下的 `.env.example` 文件，重命名为 `.env.local`：
   ```bash
   cp .env.example .env.local
   ```
   打开 `.env.local`，填入您的 API Key：
   ```env
   GEMINI_API_KEY=您的Gemini_API_Key
   ```

4. **数据库自动迁移**
   ToonTalk AI 使用本地嵌入式的 SQLite 数据库 (`local.db`)。
   - **零配置**：在第一次启动开发服务器时，系统将**自动在根目录下创建并初始化 `local.db`**，并填充预设的卡通角色、示例帖子和默认游戏商品。
   - **手动导入（可选）**：如果想要初始化更多的 Supabase 部分或进行手动重置，可以参考 `chat_schema.sql` 和 `migrations/` 文件夹。
...
4. **联系作者 / 社区微信群**：
   - *（可根据您的具体开源规划在此补充具体的邮箱、Discord 邀请链接或微信公众号二维码等）*
