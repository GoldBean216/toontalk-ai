...
export const MyHighNotes: React.FC<MyHighNotesProps> = ({ 
  posts, 
  currentUser, 
  onBack, 
  onLike,
  ...
  return (
    <div className="flex flex-col h-full bg-indigo-50 overflow-hidden">
      ...
      <div className="bg-white border-b-4 border-black p-4 sticky top-0 z-10 flex items-center shadow-md">
        ...
        <h1 className="text-2xl font-black tracking-wider uppercase">My High Notes</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-4">
        {myPosts.length === 0 ? (
            <div className="text-center mt-20 opacity-50">
                ...
                <p>Share something in the Explore feed!</p>
            </div>
        ) : (
            myPosts.map(post => {
                ...
                return (
                    <div key={post.id} className="bg-white border-4 border-black rounded-3xl p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        ...
                        <div className="flex items-center gap-4 border-t-2 border-gray-100 pt-3">
                            <button className={`flex items-center gap-1 font-bold ${isLiked ? 'text-pink-500' : 'text-gray-400'}`}>
                                <span className="text-xl">♥</span> {post.likes}
                            </button>
                            
                            <button className={`flex items-center gap-1 font-bold ${isDisliked ? 'text-black' : 'text-gray-400'}`}>
